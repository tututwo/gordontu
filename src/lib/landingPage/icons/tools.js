 ...
 * glides rigidly, the cards fill whatever room it has, and only the wrap snaps.
 */

/** Window: width (along the screen, z) and height for desktop and phone; depth; title bar. */
const DESKTOP = { w: 0.9, h: 0.72, margin: 0.1 };
const PHONE = { w: 0.5, h: 0.95, margin: 0.09 };
const DEPTH = 0.1;
const TITLE = 0.17;
 ...
 * @param {number} w @param {number} h @param {number} margin @param {boolean} column
 */
function layout(w, h, margin, column) {
	const top = h / 2 - TITLE - margin;
...
export function createToolsIcon(stage) {
	...
	const glass = stage.inked({ polygonOffset: true, polygonOffsetFactor: 1, polygonOffsetUnits: 1 });
	const bar = stage.box([DEPTH, TITLE, 1], [0, 0, 0], glass);
	const body = stage.box([DEPTH, 1, 1], [0, 0, 0], glass);
	const cards = [stage.box([CARD, 1, 1]), stage.box([CARD, 1, 1])];
	tool.add(bar, body, ...cards);
	...
	return {
		frame(dt, { lit, reduced }) {
			...
			const w = DESKTOP.w + (PHONE.w - DESKTOP.w) * t;
			const h = DESKTOP.h + (PHONE.h - DESKTOP.h) * t;
			const margin = DESKTOP.margin + (PHONE.margin - DESKTOP.margin) * t;
			bar.scale.set(DEPTH, TITLE, w);
			bar.position.set(0, h / 2 - TITLE / 2, 0);
			body.scale.set(DEPTH, h - TITLE, w);
			body.position.set(0, -TITLE / 2, 0);
			...
			const rects = layout(w, h, margin, shown);
