...
**Contact page**:
The landing's contact tab: an invitation to write about a project, a collaboration or a question, Gordon's email address (with a copy button), a form, and the names he has worked with (VISA from the bio, then the Projects' Clients). The site is static, so the form sends nothing itself: it writes the message out (name, email, how they found him, subject, message) and opens it in the visitor's own email app, addressed to him, and keeps a copy to paste if no app opened.
...
**Bio revision**:
The about bio's self-correction, played on every arrival: its second sentence first reads plainly as a tool list ("I use d3.js, three.js…"); then a strikethrough sweeps across the list in one left-to-right stroke (if the list wraps, the stroke runs on from line to line in reading order, like a pen crossing out part of one sentence), and "Claude Code, Codex & Jev across my toolkit…" is typed in after it, so the sentence now reads "I use Claude Code, Codex & Jev…". Styled like a suggested edit, but in grey only. A beat (1.5 s) after it, the bio's last line, the afterthought "Alright, you really wanna know ha: I practice Chen- and Yang-style tai chi, play acoustic guitar, and I’m learning tango with my neighbor.", comes in scrambled, word by word, left to right: each word flickers through random letters and settles on a lighter-grey jumble of about its own width, capitals and punctuation kept, and stays that way: only the avatar's Glasses read it. A pointer over the scramble, or a tap on it, nudges the avatar: he glances down at it and lifts his Glasses a little way off, then puts them back. Once they have read it, letting go of them decodes the whole line where it stands, word by word as it came in; it then stays decoded on that browser for a day, and comes in decoded on arrival. Reduced motion shows the finished revision and the scramble at once, and decodes it at once.
_Avoid_: Typo, draft, tracked change

**Glasses**:
The avatar's glasses, which come off. Pressing the avatar lifts them off to one side, bigger, as if held up to the visitor; the head turns to follow them and startles (three strokes by the hair). Dragging carries them anywhere on the page (scrolling it on at the window's edge), and through their lenses the Bio revision's scrambled last line reads plainly, each word decoding (flickering, then settling) as a lens comes over it; the moment a lens is over them, the three strokes stand up one after another into three exclamation marks (on a phone, as soon as he looks down after the glasses). While they are held, the head looks up at the pointer when it is within 75° of straight above it, and down when it is within 75° of straight below, bending into the drawings of him looking up and looking down; looking up, he does not startle. A finger carries them higher above it than a mouse does, clear of the fingertip. Let go, they spring back on and the head turns back (and if they read the scramble, it decodes for good). Worn, the glasses leave the head as it is.
