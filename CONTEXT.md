...
**Category link**:
One of the three links set into the landing headline, one per Project category: a line-drawn 3D icon inside a bracket-cornered frame, followed by the category's name, leading to that category's Postcard gallery. On hover the icon doubles in size as a white card and the sentence opens up around it like a paragraph around a tall picture (the rest of its line moves along, the lines below part), its brackets spring outward, its lines turn violet and it plays its own reveal, and a black bar wipes across the name. Leaving reverses it. Keyboard focus plays it the same way; on touch, pressing plays it and releasing follows the link. The icons: Interactive maps is a folded map that unfolds into a zigzag with contour lines flowing across it; Visual stories is a book that opens and lets cubes rise out of it and melt into one another; Web tools is a browser window that narrows from desktop to phone while its cards reflow.
