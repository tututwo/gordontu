...
**Category link**:
One of the three links set into the landing headline, one per Project category: a small wireframe icon inside a bracket-cornered frame, followed by the category's name, leading to that category's Postcard gallery. On hover the brackets spring outward, the icon spins in 3D and its edges turn violet, and a black bar wipes across the name. Leaving reverses it. Keyboard focus plays it the same way; on touch, pressing plays it and releasing follows the link.
